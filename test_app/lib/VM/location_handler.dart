...
// 드랍다운 선택시 value(화면에 보이는 값) 변경,
  ...
  // 공원별 주차장 마커 만드는 함수
  createMarker(){
    parkingMarker.value= parkingInfo.map((park) => Marker(
...
// print(marker);
  ...
  changeCameraPosition(Completer<GoogleMapController> controller) async{
    final GoogleMapController mapController = await controller.future; // mapController의 type 변환 필요

    mapController.animateCamera(CameraUpdate.newCameraPosition(
      CameraPosition(target: LatLng(parkingInfo[0].lat, parkingInfo[0].lng),zoom: 13))); 
